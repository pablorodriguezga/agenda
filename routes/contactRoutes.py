...
def getContacto():
    parametros = request.args
    id = parametros['id']
    contacto = contactController.seleccionarContacto(id)
...
def updateContacto():
    ...
    result = contactController.actualizarContacto(id,nombre,apellidos,direccion,email,telefono)
    return jsonify({'result': result})
...
def deleteContacto():
    ...
    result = contactController.eliminarContacto(id_usuario,id_contacto)
    return jsonify({'result': result})